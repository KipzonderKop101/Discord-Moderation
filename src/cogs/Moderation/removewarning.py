...
class RemoveWarning(commands.Cog):
    ...
    async def removewarning(self, ctx, member: Option(discord.Member, 'The member to remove the warning from', required=True), warning_name: Option(str, 'The warning to remove (by name)', required=False), warning_index: Option(str, 'The warning to remove (by index)', required=False)):
